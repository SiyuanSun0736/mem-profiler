...
下一阶段最该做的不是更大 backbone，而是先对已经落地的第一轮扩展特征做分组消融，并只在必要时继续补剩余字段，然后在同一 pairwise 框架下重训。

优先级建议如下：

1. 验证当前已接入的 `anon/file/write/instruction fault ratio` 是否带来稳定增益
2. 验证当前已接入的 `mmap/munmap/brk/mm_syscall density` 与 `mmap_bytes_per_ms` 是否带来稳定增益
3. 验证当前已接入的 warmup / steady-state 阶段特征是否真正改善难 pair
4. 若仍有缺口，再补 `private_fault_ratio`、更细的 mm bytes 或阶段性热点位置特征
...
现在 full/curated 账本虽然有 145 个程序，但真正进入当前训练主链路的只有 1494 条 pair、129 个程序，样本量仍不够支撑完全隐式学习。显式差分能降低无谓自由度。
...
多锚点时再按 `anchor_quality`、variant 距离和 tie-aware margin 做加权聚合，并过滤明显离群的 anchor estimate。
...
1. 是否继续保留 `O0/O2/O3` 作为默认锚点集合
2. 是否对不同锚点按质量、variant 距离和分类边际做更稳的加权
3. 是否根据 query variant 的可靠性选择局部 tuned 参数，或自动回退到 `ALL`
