...
$$
y_{i,j} = \log \frac{C_j}{C_i}
$$

其中 $C_i$ 和 $C_j$ 是当前 `run_features` 中的 `total_cycles`。

这个定义来自当前数据现实，而不是理论偏好。因为现有 `wall_time_sec` 近似固定，而 `total_cycles` 才是当前最可用的固定工作量代理量。
...
$$
\hat{y}_{i,j} \approx \log \frac{C_j}{C_i}
$$
...
$$
S_k = \log \frac{C_{O0}}{C_k}
$$

成对模型输出：

$$
\hat{r}_{x,k} \approx \log \frac{C_k}{C_x}
$$
