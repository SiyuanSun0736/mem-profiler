...
def build_run_features(data_root: pathlib.Path) -> pd.DataFrame:
    """遍历所有 variant 的 manifest，聚合每条 run 的特征，返回完整 DataFrame。"""
    records: list[dict] = []

    for variant in VARIANTS:
        manifest_path = data_root / f"manifest_bcc_{variant}.jsonl"
...
def main() -> None:
    ...
    df = build_run_features(data_root)
