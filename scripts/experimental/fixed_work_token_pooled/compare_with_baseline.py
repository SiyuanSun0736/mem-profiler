#!/usr/bin/env python3
"""
Compare the pooled fixed-work-token experiment against the baseline PairTransformer.
"""
...
POOLED_ROOT = SCRIPTS_ROOT / "experimental" / "fixed_work_token_pooled"
for candidate in (SCRIPTS_ROOT, FIXED_WORK_ROOT, POOLED_ROOT):
...
from train_fixed_work_token_transformer import _collect_run_keys, _load_pairs_table, _load_run_features, predict_with_aux_np_fixed_work
from train_fixed_work_token_pooled_transformer import FixedWorkBucketPooledTokenPairTransformer
...
def _load_pooled_model(path: pathlib.Path, device: torch.device) -> tuple[FixedWorkBucketPooledTokenPairTransformer, dict[str, Any], int]:
    checkpoint = torch.load(path, map_location=device)
    hparams = checkpoint.get("hparams", {})
    ...
    model.load_state_dict(checkpoint["model_state"])
    model.eval()
    return model, checkpoint.get("fixed_work_scaler", {}), int(hparams.get("num_buckets", 6))


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare baseline PairTransformer against pooled fixed-work-bucket-token experiment",
    ...
    parser.add_argument("--experiment-model", default="train_set/fixed_work_token_pooled_transformer/model_fixed_work_token_pooled_transformer.pt")
    parser.add_argument("--experiment-eval", default="train_set/fixed_work_token_pooled_transformer/model_fixed_work_token_pooled_eval.json")
    parser.add_argument("--output-dir", default="train_set/fixed_work_token_pooled_transformer")
    ...
    experiment_model, scaler, num_buckets = _load_pooled_model(experiment_model_path, device)

    raw_token_map, _ = build_fixed_work_token_map(run_key_df, num_buckets=num_buckets)
    ...
    experiment_pred, experiment_logits = predict_with_aux_np_fixed_work(experiment_model, df_test, token_map, device)
    ...
    json_path = out_dir / "fixed_work_pooled_vs_baseline_comparison.json"
    ...
    lines = [
        ...
        "## Overall Test",
        "",
        *_report_table(["Metric", "Baseline", "FixedWorkPooled", "Delta(exp-base)"], overall_rows),
        "",
        "## Focused Slices",
        "",
    ...
    markdown_path = out_dir / "fixed_work_pooled_vs_baseline_comparison.md"
