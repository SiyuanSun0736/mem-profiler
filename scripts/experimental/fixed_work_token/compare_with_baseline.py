#!/usr/bin/env python3
"""
Compare the fixed-work-bucket-token experiment against the baseline PairTransformer.
"""
...
EXPERIMENT_ROOT = SCRIPTS_ROOT / "experimental" / "fixed_work_token"
...
from train_fixed_work_token_transformer import (
    FixedWorkBucketTokenPairTransformer,
    _collect_run_keys,
    _load_pairs_table,
    _load_run_features,
...
def _load_fixed_work_model(path: pathlib.Path, device: torch.device) -> tuple[FixedWorkBucketTokenPairTransformer, dict[str, Any], int]:
    checkpoint = torch.load(path, map_location=device)
    hparams = checkpoint.get("hparams", {})
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare baseline PairTransformer against fixed-work-bucket-token experiment",
    ...
    parser.add_argument("--experiment-model", default="train_set/fixed_work_token_transformer/model_fixed_work_token_transformer.pt")
    parser.add_argument("--experiment-eval", default="train_set/fixed_work_token_transformer/model_fixed_work_token_eval.json")
    parser.add_argument("--output-dir", default="train_set/fixed_work_token_transformer")
    ...
    experiment_model, scaler, num_buckets = _load_fixed_work_model(experiment_model_path, device)
    ...
    json_path = out_dir / "fixed_work_vs_baseline_comparison.json"
    ...
    lines = [
        ...
        "## Overall Test",
        "",
        *_report_table(["Metric", "Baseline", "FixedWorkToken", "Delta(exp-base)"], overall_rows),
        "",
        "## Focused Slices",
        "",
    ...
    markdown_path = out_dir / "fixed_work_vs_baseline_comparison.md"
