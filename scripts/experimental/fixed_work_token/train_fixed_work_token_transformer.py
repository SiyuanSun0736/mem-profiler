#!/usr/bin/env python3
"""
Fixed-work bucket token experiment.

Each run is re-parameterized by cumulative work progress (instructions share), then
split into equal-work buckets. This matches the repository's fixed-work label more
closely than fixed-time tokenization.
"""
...
class FixedWorkBucketTokenPairTransformer(nn.Module):
    def __init__(
        self,
        summary_dim: int,
        bucket_feat_dim: int,
...
def make_fixed_work_tensors(
    ...
    bi = np.stack([token_map[key] for key in keys_i], axis=0).astype(np.float32)
    bj = np.stack([token_map[key] for key in keys_j], axis=0).astype(np.float32)
...
def train_fixed_work_model(
    model: FixedWorkBucketTokenPairTransformer,
    device: torch.device,
    ...
    xi_tr, xj_tr, bi_tr, bj_tr, y_tr, ycls_tr, regw_tr = make_fixed_work_tensors(df_train, token_map, device)
    xi_va, xj_va, bi_va, bj_va, y_va, ycls_va, regw_va = make_fixed_work_tensors(df_val, token_map, device)
    ...
    loader = DataLoader(
        TensorDataset(xi_tr, xj_tr, bi_tr, bj_tr, y_tr, ycls_tr, regw_tr),
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xi_b, xj_b, bi_b, bj_b, y_b, ycls_b, regw_b in loader:
            optimizer.zero_grad()
            if noise_std > 0.0:
            ...
            pred, cls_logits = model.forward_with_aux(xi_b, xj_b, bi_b, bj_b)
        ...
        model.eval()
        with torch.no_grad():
            val_pred, val_cls_logits = model.forward_with_aux(xi_va, xj_va, bi_va, bj_va)
...
@torch.no_grad()
def predict_with_aux_np_fixed_work(
    model: FixedWorkBucketTokenPairTransformer,
    df: pd.DataFrame,
    token_map: dict[tuple[str, str], np.ndarray],
    device: torch.device,
) -> tuple[np.ndarray, np.ndarray]:
    xi, xj, bi, bj, _, _, _ = make_fixed_work_tensors(df, token_map, device)
    model.eval()
    pred, cls_logits = model.forward_with_aux(xi, xj, bi, bj)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Experimental fixed-work-bucket-token transformer training",
    ...
    parser.add_argument("--output", default="train_set/fixed_work_token_transformer")
    ...
    if len(run_key_df) != len(all_keys):
        sys.exit("[error] fixed-work token run mapping is incomplete for the active pairs table")

    schema = build_fixed_work_token_schema(num_buckets=args.num_buckets)
    schema_path = out_dir / "fixed_work_token_schema.json"
    ...
    print("\n" + sep)
    print(
        "Step 3: Build FixedWorkBucketTokenPairTransformer  "
    ...
    model_path = out_dir / "model_fixed_work_token_transformer.pt"
    torch.save(
        {
    ...
    eval_result = {
        ...
        "source_isolation": {
            ...
            "experimental_script": "scripts/experimental/fixed_work_token/train_fixed_work_token_transformer.py",
    ...
    eval_path = out_dir / "model_fixed_work_token_eval.json"
