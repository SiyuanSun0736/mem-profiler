#!/usr/bin/env python3
"""
Aggregate five token-branch comparisons into a single side-by-side report.
...
Inputs are the baseline comparison JSONs produced by:
  ...
  - fixed-work+hotspot hybrid branch
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Aggregate five token experiment branches into one report",
    ...
    parser.add_argument("--fixed-work-hotspot-json", default="train_set/fixed_work_hotspot_token_transformer/fixed_work_hotspot_vs_baseline_comparison.json")
    parser.add_argument("--output-dir", default="train_set/token_branch_comparison_all")
    ...
    fixed_work_hotspot_json = (REPO_ROOT / args.fixed_work_hotspot_json).resolve()
    ...
    payloads = {
        ...
        "fixed_work_hotspot": _load_json(fixed_work_hotspot_json),
    }
    labels = {
        ...
        "fixed_work_hotspot": "FixedWorkHotspot",
    }

    branch_order = ("category", "hotspot", "fixed_work", "fixed_work_pooled", "fixed_work_hotspot")
    ...
    summary = {
        ...
        "inputs": {
            "category": _display_path(category_json),
            "hotspot": _display_path(hotspot_json),
            "fixed_work": _display_path(fixed_work_json),
            "fixed_work_pooled": _display_path(fixed_work_pooled_json),
            "fixed_work_hotspot": _display_path(fixed_work_hotspot_json),
        },
        "overall": overall_rows,
        "focused": slice_rows,
    }
    json_path = out_dir / "token_branch_comparison_all.json"
    ...
    lines = [
        ...
        f"- fixed_work_hotspot: {summary['inputs']['fixed_work_hotspot']}",
        "",
        "## Overall Test",
        "",
    ...
    md_path = out_dir / "token_branch_comparison_all.md"
    md_path.write_text("\n".join(lines) + "\n")
