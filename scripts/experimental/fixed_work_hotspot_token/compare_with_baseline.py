#!/usr/bin/env python3
"""
Compare the fixed-work+hotspot-token experiment against the baseline PairTransformer.
"""
...
HYBRID_ROOT = SCRIPTS_ROOT / "experimental" / "fixed_work_hotspot_token"
for candidate in (SCRIPTS_ROOT, FIXED_WORK_ROOT, HOTSPOT_ROOT, HYBRID_ROOT):
...
from fixed_work_token_schema import apply_fixed_work_token_scaler, build_fixed_work_token_map
from hotspot_token_schema import apply_hotspot_token_scaler, build_hotspot_token_map
from train_fixed_work_hotspot_token_transformer import (
    FixedWorkHotspotTokenPairTransformer,
    _collect_run_keys,
    _load_pairs_table,
    _load_run_features,
    predict_with_aux_np_fixed_work_hotspot,
)
...
def _load_hybrid_model(
    path: pathlib.Path,
    device: torch.device,
) -> tuple[FixedWorkHotspotTokenPairTransformer, dict[str, Any], dict[str, Any], int, int]:
    checkpoint = torch.load(path, map_location=device)
    hparams = checkpoint.get("hparams", {})
    model = FixedWorkHotspotTokenPairTransformer(
        summary_dim=int(hparams.get("summary_dim", len(NON_TIME_COLS))),
        bucket_feat_dim=int(hparams.get("bucket_feat_dim", 10)),
        window_feat_dim=int(hparams.get("window_feat_dim", 10)),
        num_buckets=int(hparams.get("num_buckets", 6)),
    ...
    return (
        model,
        checkpoint.get("fixed_work_scaler", {}),
        checkpoint.get("hotspot_scaler", {}),
        int(hparams.get("num_buckets", 6)),
        int(hparams.get("top_k", 6)),
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare baseline PairTransformer against fixed-work+hotspot-token experiment",
    ...
    parser.add_argument("--experiment-model", default="train_set/fixed_work_hotspot_token_transformer/model_fixed_work_hotspot_token_transformer.pt")
    parser.add_argument("--experiment-eval", default="train_set/fixed_work_hotspot_token_transformer/model_fixed_work_hotspot_token_eval.json")
    parser.add_argument("--output-dir", default="train_set/fixed_work_hotspot_token_transformer")
    ...
    experiment_model, fixed_work_scaler, hotspot_scaler, num_buckets, top_k = _load_hybrid_model(experiment_model_path, device)

    raw_fixed_work_token_map, _ = build_fixed_work_token_map(run_key_df, num_buckets=num_buckets)
    raw_hotspot_token_map, _ = build_hotspot_token_map(run_key_df, top_k=top_k)
    ...
    experiment_pred, experiment_logits = predict_with_aux_np_fixed_work_hotspot(
        experiment_model,
        df_test,
    ...
    summary = {
        ...
        "baseline": {
            "model_path": _display_path(baseline_model_path),
            "eval_path": _display_path(baseline_eval_path),
            "model_name": baseline_eval.get("model"),
            "n_params": baseline_eval.get("n_params"),
        },
        "experiment": {
            "model_path": _display_path(experiment_model_path),
            "eval_path": _display_path(experiment_eval_path),
    ...
    json_path = out_dir / "fixed_work_hotspot_vs_baseline_comparison.json"
    ...
    lines = [
        ...
        "## Overall Test",
        "",
        *_report_table(["Metric", "Baseline", "FixedWorkHotspot", "Delta(exp-base)"], overall_rows),
        "",
        "## Focused Slices",
        "",
    ...
    markdown_path = out_dir / "fixed_work_hotspot_vs_baseline_comparison.md"
