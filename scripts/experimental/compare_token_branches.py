#!/usr/bin/env python3
"""
Aggregate token-branch comparisons into a single side-by-side report.

Inputs are the baseline comparison JSONs already produced by the category-token,
hotspot-token, and fixed-work-token branches.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Aggregate token experiment branches into one report",
    ...
    parser.add_argument("--output-dir", default="train_set/token_branch_comparison")
    ...
    overall_rows = []
    for key in ("category", "hotspot", "fixed_work"):
    ...
    json_path = out_dir / "token_branch_comparison.json"
    ...
    md_path = out_dir / "token_branch_comparison.md"
    md_path.write_text("\n".join(lines) + "\n")
